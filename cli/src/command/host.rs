...
fn setup_crate(host_path: PathBuf) -> anyhow::Result<()> {
    ...
    // add serde and postcard because they are used for (de)serializing from/to the input/output tapes
    cargo(Some(&guest_path), ["add", "postcard", "-F", "alloc"])?;
    cargo(Some(&guest_path), ["add", "serde", "--no-default-features"])?;
