...
pub fn generate_main_trace(side_note: &mut SideNote) -> FinalizedTrace {
    ...
    for (row_idx, program_step) in side_note.iter_program_steps().enumerate() {
        generate_trace_row(&mut trace, row_idx, program_step, &mut range_check_mults);
    }
    side_note.range_check.range64.append(range_check_mults);
...
fn generate_trace_row(
    ...
    range_check_accum: &mut RangeCheckMultiplicities<64>,
) {
    ...
    range_check_accum.add_value(pc_aux);
}
