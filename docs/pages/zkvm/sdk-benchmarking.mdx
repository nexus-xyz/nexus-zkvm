...
The host program `src/main.rs` is:
...
The guest program `src/guest/src/main.rs` is:
...
Rust can automatically optimize the code for the current CPU architecture.
In this example, we use the `-C target-cpu=native` flag to build the host program for the native CPU to optimize the performance.
If you want to set a specific CPUs, 
you can obtain the list of target CPUs can be obtained from this command `rustc --print target-cpus`.

```rust 
RUSTFLAGS='-C target-cpu=native' cargo build --release --package cargo-nexus --bin cargo-nexus
```
...
Using the `#[nexus_rt::profile]` macro, the example output from `cargo-nexus nexus run` for the 10th Fibonacci number is:
...
3. **Optimize Proving Time**: While proving time increases significantly with program complexity, verification time remains relatively constant. This suggests that optimizing for faster proving will yield the most significant performance improvements.

4. **Leverage Performance Insights**: Use the performance characteristics revealed in this benchmark to make informed decisions about program design and optimization strategies. Understanding the relationship between guest cycles and host proving time can guide Nexus zkVM development process.

5. **Profile Regularly**: Regularly profile your Nexus zkVM projects using tools like `#[nexus_rt::profile]` to identify performance bottlenecks and opportunities for optimization.

By keeping these suggestions in mind, developers can create more efficient and performant applications using Nexus zkVM, ensuring better scalability and user experience in zero-knowledge proof systems.
