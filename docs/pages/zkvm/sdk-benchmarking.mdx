...
If you want to set a specific CPU, 
the list of available target CPUs can be obtained with the command  `rustc --print target-cpus`.
...
In the `src/guest/` directory, run the command: `cargo-nexus nexus run 2>/dev/null`
