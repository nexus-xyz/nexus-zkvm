...
The graphs below illustrate the correlation between RISC-V cycles and prover time for the Fibonacci sequence calculation.

1. The number in each bar represents the total execution time in seconds, which includes:
   - Generating Public Parameters
   - Proving
   - Verifying
2. Verification time is negligible compared to the prover time.

3. Public parameters generation time remains constant at approximately 25 seconds across all iterations.
