...
In this example, we use the `-C target-cpu=native` flag to build `cargo-nexus` for the native CPU to optimize performance.
...
We will use `cargo-nexus` to build the guest program in this example.
