...
### v1.2.0 to Beyond
This documentation is for the Nexus v.1.1.0 system. Many improvements to individual components are incoming, to be featured in the Nexus v.1.2.0 system and future upgrade cycles beyond. Expected improvements include:
...
2. **Proof Compression**: A second phase of proof compression, on top of the v1.1.0 $O(log(n))$ proof compression mechanism, which will bring the proof to $O(log(log(n)))$. That is, a proof of a proof of a proof.
