...
impl MachineChip for BneChip {
    ...
    fn add_constraints<E: EvalAtRow>(
        ...
        let (_, neq_flag) = trace_eval!(trace_eval, Column::Neq);
        let (_, neq_12_flag) = trace_eval!(trace_eval, Column::Neq12);
        let (_, neq_34_flag) = trace_eval!(trace_eval, Column::Neq34);
        ...
        let (_, is_bne) = trace_eval!(trace_eval, Column::IsBne);
        let is_bne = is_bne[0].clone();

        let (_, neq_12_flag_aux) = trace_eval!(trace_eval, Column::Neq12Aux);
        let (_, neq_34_flag_aux) = trace_eval!(trace_eval, Column::Neq34Aux);
        let (_, neq_12_flag_aux_inv) = trace_eval!(trace_eval, Column::Neq12AuxInv);
        let (_, neq_34_flag_aux_inv) = trace_eval!(trace_eval, Column::Neq34AuxInv);
        ...
        // is_bne・(neq_flag・c_val_1 + (1-neq_flag)・4 + pc_1 - carry_1·2^8 - pc_next_1) = 0
        eval.add_constraint(
            is_bne.clone()
                * (neq_flag[0].clone() * value_c[0].clone()
                    + (E::F::one() - neq_flag[0].clone()) * E::F::from(4u32.into())
                    + pc[0].clone()
                    - carry_bits[0].clone() * modulus.clone()
                    - pc_next[0].clone()),
        );

        // is_bne・(neq_flag・c_val_2 + pc_2 + carry_1 - carry_2·2^8 - pc_next_2) = 0
        // is_bne・(neq_flag・c_val_3 + pc_3 + carry_2 - carry_3·2^8 - pc_next_3) = 0
        // is_bne・(neq_flag・c_val_4 + pc_4 + carry_3 - carry_4·2^8 - pc_next_4) = 0
        for i in 1..WORD_SIZE {
            eval.add_constraint(
                is_bne.clone()
                    * (neq_flag[0].clone() * value_c[i].clone()
                        + pc[i].clone()
...
mod test {
    ...
    fn test_k_trace_constrained_bne_instructions() {
        ...
        for (row_idx, program_step) in program_steps.enumerate() {
            ...
            BneChip::fill_main_trace(&mut traces, row_idx, &program_step, &mut side_note);
        ...
        assert_chip::<(CpuChip, AddChip, SubChip, BneChip, RegisterMemCheckChip)>(
            traces,
