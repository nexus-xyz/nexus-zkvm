...
impl MachineChip for TimestampChip {
    ...
    fn add_constraints<E: EvalAtRow>(
        ...
        let (_, ch1_minus) = trace_eval!(trace_eval, CH1Minus);
        let (_, ch2_minus) = trace_eval!(trace_eval, CH2Minus);
        let (_, ch3_minus) = trace_eval!(trace_eval, CH3Minus);
        ...
        let (_, reg1_ts_prev) = trace_eval!(trace_eval, Reg1TsPrev);
        let (_, reg2_ts_prev) = trace_eval!(trace_eval, Reg2TsPrev);
        let (_, reg3_ts_prev) = trace_eval!(trace_eval, Reg3TsPrev);
        let (_, reg1_ts_cur) = preprocessed_trace_eval!(trace_eval, PreprocessedColumn::Reg1TsCur);
        let (_, reg2_ts_cur) = preprocessed_trace_eval!(trace_eval, PreprocessedColumn::Reg2TsCur);
        let (_, reg3_ts_cur) = preprocessed_trace_eval!(trace_eval, PreprocessedColumn::Reg3TsCur);
        let (_, c_reg1_ts_prev) = trace_eval!(trace_eval, CReg1TsPrev);
        let (_, c_reg2_ts_prev) = trace_eval!(trace_eval, CReg2TsPrev);
        let (_, c_reg3_ts_prev) = trace_eval!(trace_eval, CReg3TsPrev);
